 ...
 * @return node
 */
inline _tbm_node * _tbm_lookup(uint32_t prefix, uint8_t prefix_len, uint16_t * index)
{
	uint16_t bit_value;
	uint8_t length;
	uint8_t position = 0;
...
void lpm_init(_LPM_RULE default_rule, _LPM_RULE default_rule6)
{
	...
	_tbm_zeros(_tbm_root->external, _TBM_SIZE_EXTERNAL);
	_tbm_zeros(_tbm_root->internal, _TBM_SIZE_INTERNAL);
