...
void _nfa_state_destroy(stack * already_freed, _nfa_state * state)
{
	...
	*/
	...
	stack_push_unique(already_freed, state, POINTER);

	for(unsigned i = 0; i < length; ++i)
	{
		if(stack_contains(already_freed, next[i], POINTER)) continue;
		_nfa_state_destroy(already_freed, next[i]);
	}
	...
	{
		if(stack_contains(already_freed, next_epsilon[i], POINTER)) continue;
		_nfa_state_destroy(already_freed, next_epsilon[i]);
	}
...
void _dfa_state_destroy(stack * already_freed, _dfa_state * state)
{
	...
	*/
	...
	stack_push(already_freed, state, POINTER);

	for(unsigned i = 0; i < length; ++i)
	{
		if(stack_contains(already_freed, next[i], POINTER)) continue;
		_dfa_state_destroy(already_freed, next[i]);
	}
...
{
	stack * already_freed = stack_init();

	_dfa_state_destroy(already_freed, state);
	stack_destroy(already_freed);
}
...
_dfa_pair * _dfa_create_pair(stack ** nfa_states)
{
	...
	*nfa_states = tmp;
	...
	for(unsigned i = 0; i < pair->nfa_states->end; ++i)
	{
...
unsigned _precedence(stack * stack_, short current)
{
	...
	short top = stack_top_type(stack_, NUMBER).number;

	return precedence_table[current - 300][top - 300];
}
...
_Bool _dfa_epsilon_closure(stack * nfa, stack * closure)
{
	...
	for(unsigned i = 0; i < closure->end; ++i)
	{
		...
		for(unsigned i = 0; i < length; ++i)
		{
			next = ((_nfa_state *) item)->next_epsilon[i];
			if(stack_push_unique(closure, next, POINTER) == 0) return 0;
		}
	}
...
{
	_nfa_state * state, * next;
	void * position;

	for(unsigned i = 0; i < input_states->end; ++i)
	{
		state = input_states->data[i].value.pointer;

		if((position = memchr(state->key, character, state->length)) != NULL)
		{
			next = state->next[(char *) position - state->key];
			if(stack_push_unique(output_states, next, POINTER) == 0) return 0;
		}
	}
...
_Bool _dfa_match_processed(stack_value first, stack_value second)
{
	...
	if(first_stack->end != second_stack->end) return 0;

	for(unsigned i = 0; i < first_stack->end; ++i)
	{
		if(!stack_contains(second.pointer, first_stack->data[i].value.pointer, POINTER)) return 0;
	}
 ...
 regex_nfa * _nfa_construct(regex_pattern pattern)
{
	...
	if(stack_push(stack_, STACK_BOTTOM, NUMBER) == 0) return _nfa_clear(stack_);
	...
	while(current != END)
	{
		...
		do
		{
			...
			{
				block = stack_pop(stack_).pointer; // pop block
				stack_pop(stack_); // pop OPEN_PAREN
				if(stack_push(stack_, block, POINTER) == 0) return _nfa_block_destroy(block), _nfa_clear(stack_);
				prev = CLOSE_PAREN;
			}
			...
			switch(current)
			{
				...
				case DOT:
					prev = DOT;
					block = _nfa_create_dot_block();
					if(block == NULL) return _nfa_clear(stack_);
					if(stack_push(stack_, block, POINTER) == 0) return _nfa_block_destroy(block), _nfa_clear(stack_);
					break;

				case OPEN_BRACKET:
					block = _enumeration(input, length, &position);
					if(block == NULL) return _nfa_clear(stack_);
					if(stack_push(stack_, block, POINTER) == 0) return _nfa_block_destroy(block), _nfa_clear(stack_);
					prev = CLOSE_BRACKET;
					break;
				...
				case PLUS:
					prev = current;
					if(stack_push(stack_, current, NUMBER) == 0) return _nfa_clear(stack_);
					break;

				default:
					block = _nfa_create_block(current);
					if(block == NULL) return _nfa_clear(stack_);
					if(stack_push(stack_, block, POINTER) == 0) return _nfa_block_destroy(block), _nfa_clear(stack_);
					prev = SYMBOL;
					break;
...
regex_dfa * _nfa_convert(regex_nfa * root)
{
	...
	if(stack_push(nfa_list, root, POINTER) == 0) return _DFA_CLEAR(1);
	...
	if((stack_push(pairs_unprocessed, pair, POINTER)) == 0) return _DFA_CLEAR(1);
	if((stack_push(pairs_created, pair, POINTER)) == 0) return _DFA_CLEAR(1);
	...
	while(!stack_empty(pairs_unprocessed))
	{
		...
		for(int i = CHAR_MIN; i <= CHAR_MAX; ++i)
		{
			...
			if(existing_pair == NULL)
			{
				...
				if(stack_push(pairs_unprocessed, new_pair, POINTER) == 0) return _dfa_pair_free(new_pair), _DFA_CLEAR(1);
				if(stack_push(pairs_created, new_pair, POINTER) == 0) return _DFA_CLEAR(1);
				if(_dfa_add_transition(pair->dfa_state, i, new_pair->dfa_state) == 0) return _DFA_CLEAR(1);

			}
			else
			{
				closure_states->end = 0;
				if(_dfa_add_transition(pair->dfa_state, i, ((_dfa_pair *) existing_pair)->dfa_state) == 0) return _DFA_CLEAR(1);
			}
		}
	}
...
int regex_match_nfa(regex_nfa * regex, char * input, unsigned length)
{
	...
	if(stack_push(start, regex, POINTER) == 0) return errno = FASTNET_OUT_OF_MEMORY, stack_destroy(start), stack_destroy(end), NOT_MATCH;
	...
	while(position < length)
	{
		while(!stack_empty(start))
		{
			...
			{
				if(stack_push_unique(start, state->next_epsilon[i], POINTER) == 0) return errno = FASTNET_OUT_OF_MEMORY, stack_destroy(start), stack_destroy(end), NOT_MATCH;
			}

			if((char_position = memchr(state->key, input[position], state->length))) // OR final state
			{
				if(stack_push_unique(end, state->next[char_position - (void *) state->key], POINTER) == 0) return errno = FASTNET_OUT_OF_MEMORY, stack_destroy(start), stack_destroy(end), NOT_MATCH;
			}
		}

		if(stack_empty(end))
		{
			if(stack_push_unique(end, regex, POINTER) == 0) return errno = FASTNET_OUT_OF_MEMORY, stack_destroy(start), stack_destroy(end), NOT_MATCH;
		}
