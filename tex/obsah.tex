...
\chapter{Nějaký text, snad něco půjde použít}

\section{Internet}

\subsection{IP adresa}
IP adresa číslo, které jednoznačně identifikuje zařízení v počítačové síti založené na adresním protokolu IP.
V době psaní této práce jsou používány dvě verze tohoto protokolu a to IPv4 a IPv6.

\subsubsection{IPv4}
První verze IP protokolu, IPv4, používá pro adresaci 32 bitů. Těchto 32 bitů se zapisuje jako čtveřice čísel oddělených tečkou, kde každá čtveřice může obsahovat číslo v rozsahu $<0, 255>$ neboli $2^8$ - počet všechno možných kombinací binárního čísla o délky 8 bitů, jeden byte.
TODO speciální adresy

\subsubsubsection{Třídní adresy}
Třídní adresy se použivali v době kdy internet ještě nebyl široce rozšířený a nebylo potřeba tolik adres.
Adresy se dělí do čtyř tříd kdy každá třída je označena písmenem abecedy A-E.
Podle tohoto písmena se odvíjela i maska sítě, která měla vždy délku násobků 8.

V následující tabulce jsou masky pro jednotlivé třídy adres

A /8 255.0.0.0 počet sítí počet hostů v každé síti
B /16 255.255.0.0 počet sítí počet hostů v každé síti
C /24 255.255.255.0 počet sítí počet hostů v každé síti
D / special
E / special reserved

V těchto rozsazích se také nacházejí speciální adresy, jejichž přehled je uveden v následující tabulce

Třída, rozsah, maska, význam
A 10.0.0.0/8, 255.0.0.0, privátní ip adresy, nesměrovatelné v internetu
A 127.0.0.1 - loopback
B 172.16/12, 255.224.0.0 privátní ip adresy, nesměrovatelné v internetu
C 192.168.0.0/16 255.255.255.0 privátí
  224/x multicast
E experimentální

Při přídním adresování dochází k plýtvání místem - pokud chceme síť pro 50 zařízení nejmenší možná síť, kterou můžeme použít je pro 254 hostů.

\subsubsubsection{Beztřídní adresy - VLSM}
VLSM - variable length subnet mask neboli masky proměnné délky se začali používat v době masivního rozšíření internetu, kdy bylo jasné, že tzv třídní adresy nemohou pokrýt kvantitativní požadavky na ip adresy.

Beztřídní adresy umožňují vytvářet sítě v jakoukoli maskou - maska již dále není závislá na prvních bitech prvního octetu.
To má několik výhod - sítě se mohou vytvářet tak velké, jak jsou potřeba a nedochází k plýtvání adres.
Ale má to i své nevýhody - adresy sítí už nejsou zřejmé a v routovacích tabulkách se musí uchovávat i maska sítě, což má za následek zvětšení místo zabíraného routovací tabulkou a zpomalení vyhledávání v jejích strukturách

\subsubsection{IPv6}
IPv6 je IP adresa délky 128bitů. Pro ipv6 existuje několik způsobů zápisu adresy.


nejdelší množinu po sobě jdoucích 0 lze vynechat úplně vynechat a celý vynechaný úsek nahradit pouze čtyřtečkou.

ipv6 má tak velký adresový prostor, že ho lze považovat za nevyčerpatelný.
Při použití adres s maskou 40 vychází na každého člověka 3 bilióny ipv6 adres.

\subsection{Směrování}

\subsubsection{CIDR}
cidr je pravým opakem vlsm. Pro několik sití jež mohou mít různou délku prefixu vybírá nejdelší shodnout část adresy a pod touto adresou poté směruje veškerý provoz, který je určený pro tyto sítě. Cidr se využívá na routerech a důvodem pro jeho zavedení bylo zmenšení obsahu směrovacích tabulek na směrovačích - routrech.

\section{LPM - longest prefix match}
Operace vyhledání nejdelšího shodného prefixu je velice častá operace, která se používá zejména při směrování v počítačových sítích.
Vyhledání nejdelšího prefixu se používá pro vyhledání nejpřesnějšího záznamu v tzv routovací tabulce. Routovací tabulka jak již název napovídá, je tabulka, která obsahuje ntici záznamů, které se kládají minimálně z informací ip adresa, maska ip adresy/prefix a výstupní port zařízení.
Při masivním rozšíření internetu se začalo používat tzv beztřídní směrování.

\subsection{BSPL}
Binary search on prefix length - binární vyhledávání dle délky prefixu.
Pro uložení vyhledávácích struktur využívá dvě základní programové struktury a těmi jsou binární strom a hešovací tabulka.
V binárním stromu jsou uloženy informace potřebné pro přidávání, aktualizace a mazání jednotlivých adres, hešovací tabulka je pak používáná pouze pro operaci vyhledání nejdelší shody.
\subsubsection{Optimalizace}
Jako optimalizaci jsem se rozhodl ze struktury binárního stromu sjednotit ukazatele na potomky pouze do ukazatele jednoho
Tuto optimalizace jsi mohu dovolit z důvodu operace zvané leaf pushing, která zajišťuje že pokud má uzel stromu potomka musí mít potomky oba s tím, že potomek který by bez této operace neexistoval bude zdědí hodnotu nadřazeného uzlu. Tím je zajištěno že při této optimalizace nebudou vznikat uzly, které by bez ní neexistoval a ušetří se tím položka o velikosti ukazatel - standradně 4B - pro každý uzel stromové struktury
Vyhledávání:
Pro zadanou adresu je
